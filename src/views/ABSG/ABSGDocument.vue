    ...
    <div class="flex gap-5 my-10 flex-col md:flex-row">
      ...
      <template v-if="isMedia">
        <div  class="md:w-9/12 lg:w-9/12 xl:w-10/12 shrink-0 grow-0">
          ...
          <div>
            <ABSGDocumentAudio v-if="segmentName === 'audio'" :audio="audio" :resource="resource" :document="document"></ABSGDocumentAudio>
...
import ABSGDocumentAudio from '@/components/ABSG/ABSGDocumentAudio.vue'
import ABSGDocumentVideo from '@/components/ABSG/ABSGDocumentVideo.vue'
...
export default {
  ...
  data () {
    return {
      ...
      lessonsOpened: true,
      teacherHelpsOpened: false,
      mediaOpened: false,

      teacherCommentsEnabled: false,
      hopeSabbathSchoolOutlineEnabled: false,
      hopeSabbathSchoolVideoEnabled: false,
      talkingPointsOutlineEnabled: false,
      talkingPointsVideoEnabled: false,

      hopeSabbathSchoolOpened: false,
      talkingPointsOpened: false,
    ...
    async getDocument () {
      ...
      try {
        ...
        if (/\d{4}-\d{2}/.test(resourceName)) {
          const [yearStr, quarterStr] = resourceName.split('-');
        ...
        if (video.find((v) => /Hope Sabbath School/.test(v.artist))) {
          this.hopeSabbathSchoolVideoEnabled = true
        }

        if (video.find((v) => /Talking Points/.test(v.artist))) {
          this.talkingPointsVideoEnabled = true
        }
      } catch (e) {}

      try {
        if (this.document.segments[0].blocks.find((b) => b.type === 'reference' && b.target.indexOf('/teacher-comments') > 0)) {
          this.teacherCommentsEnabled = true
        }

        if (this.document.segments[0].blocks.find((b) => b.type === 'reference' && b.target.indexOf('/hope-ss') > 0)) {
          this.hopeSabbathSchoolOutlineEnabled = true
        }

        if (this.document.segments[0].blocks.find((b) => b.type === 'reference' && b.target.indexOf('/_talking-points') > 0)) {
          this.talkingPointsOutlineEnabled = true
        }
      } catch (e) {}
      ...
      if (['teacher-comments', 'hope-ss', 'talking-points', 'hope-ss-videos', 'talking-points-videos', 'more-videos'].includes(segmentName)) {
        this.segmentName = segmentName

        if (segmentName === 'talking-points' || segmentName === 'talking-points-videos') {
          if (segmentName === 'talking-points') {
            segmentName = '_talking-points'
          }
          this.talkingPointsOpened = true
        }

        if (segmentName === 'hope-ss' || segmentName === 'hope-ss-videos') {
          this.hopeSabbathSchoolOpened = true
        }

        if (/-videos$/.test(segmentName) && this.video) {
          this.videoEnabled = true
          this.lessonsOpened = false
          this.teacherHelpsOpened = true

          if (/talking-points/.test(segmentName)) {
          ...
          try {
            this.segment = (await this.$apiResources.get(`en/${resourceType}/${resourceName}/${documentName}/${segmentName}/index.json`)).data
            this.lessonsOpened = false
            this.teacherHelpsOpened = true
          } catch (e) {
            console.error(e)
